...
async def count_mentions():
    ...
    # Gather all messages within the specified time range
    channel = bot.get_channel(channel_id)
    messages = []
    async for message in channel.history(limit=None, after=start_time, before=end_time):
        messages.append(message)

    # Process mentions in bulk
    for message in messages:
        for user in message.mentions:
            mention_counts[message.author.id] += 1
            unique_mention_counts[message.author.id].add(user.id)

    # Sort the mention_counts and unique_mention_counts dictionaries by the number of unique mentions
    sorted_mentions = sorted(mention_counts.items(), key=lambda x: len(unique_mention_counts[x[0]]), reverse=True)
...
async def top_mentions(ctx):
    ...
    channel = bot.get_channel(channel_id)
    async for message in channel.history(limit=None, after=last_sunday, before=now):
